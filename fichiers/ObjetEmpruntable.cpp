...
//o Une fois déterminé, la méthode doit faire un dynamic_cast<> pour obtenir un
//  pointeur de la classe appropriée, dans le but d’appeler l’opérateur << de la sousclasse
//  appropriée.Ex dynamic_cast :
//  Classe_Derivé* = dynamic_cast<Classe_Derive*> (Classe_Base*);
//o Puisque this est un pointeur constant, alors assurez - vous d’utiliser le mot - clé
//	const lors de la conversion du type de pointeur.
void ObjetEmpruntable::afficherObjetEmpruntable(std::ostream& o) {
	//WTF CALICE
	string classe = obtenirNomClasse();

	if (classe == "livre") {
		ObjetEmpruntable* livre = new ObjetEmpruntable();
		Livre* livre = dynamic_cast<Livre*> (livre);
	}
	if(classe == "dvd")
	if(classe == "livreNumerique")
